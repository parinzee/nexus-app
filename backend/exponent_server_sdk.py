...
class PushTicket(
    namedtuple("PushTicket", ["push_message", "status", "message", "details", "id"])
):
...
class PushReceipt(namedtuple("PushReceipt", ["id", "status", "message", "details"])):
...
class PushClient(object):
    ...
    def __init__(self, host=None, api_url=None, session=None, **kwargs):
        ...
        self.max_message_count = (
            kwargs["max_message_count"]
            if "max_message_count" in kwargs
            else PushClient.DEFAULT_MAX_MESSAGE_COUNT
        )
        self.max_receipt_count = (
            kwargs["max_receipt_count"]
            if "max_receipt_count" in kwargs
            else PushClient.DEFAULT_MAX_RECEIPT_COUNT
        )
    ...
    def is_exponent_push_token(cls, token):
        ...
        return isinstance(token, six.string_types) and token.startswith(
            "ExponentPushToken"
        )
    ...
    def publish_multiple(self, push_messages):
        ...
        for start in itertools.count(0, self.max_message_count):
            chunk = list(
                itertools.islice(push_messages, start, start + self.max_message_count)
            )
            if not chunk:
                break
    ...
    def check_receipts_multiple(self, push_tickets):
        ...
        for start in itertools.count(0, self.max_receipt_count):
            chunk = list(
                itertools.islice(push_tickets, start, start + self.max_receipt_count)
            )
            if not chunk:
                break
